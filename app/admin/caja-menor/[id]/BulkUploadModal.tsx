...
export function BulkUploadModal({ boxId, onClose, onSuccess }: BulkUploadModalProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await fetch('/api/caja-menor/bulk-upload', {
        method: 'POST',
        body: formData,
      });
      ...
      if (res.ok) {
        ...
        // Recargar la tabla para mostrar las nuevas facturas
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-[var(--bg)] border border-[var(--border)] rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="p-6 border-b border-[var(--border)] flex justify-between items-center bg-[var(--surface)]">
          <div>
            <h2 className="text-xl font-bold text-[var(--fg)]">Carga Masiva de Facturas</h2>
            <p className="text-sm text-[var(--dim)] mt-1">Sube múltiples recibos a Drive. La IA extraerá los datos automáticamente.</p>
          </div>
