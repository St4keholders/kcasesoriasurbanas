        ...
        try {
          if (process.env.OPENAI_API_KEY) {
            ...
            let messages: any[] = [
              {
                role: 'system',
                content: 'Eres un experto contable. Extrae los datos de esta factura o recibo. Devuelve SOLO un objeto JSON válido con los siguientes campos y tipos exactos: supplier_name (string), supplier_document (string, NIT/Cedula), invoice_number (string), transaction_date (YYYY-MM-DD, extrae cuidadosamente la fecha real de la transacción), amount (numero sin formato, corresponde al subtotal. Si no hay desglose de impuestos, asigna el gran total pagado a este campo amount), tax_iva (numero sin formato, valor del IVA), withholding_tax (numero sin formato, valor de la retención en la fuente, 0 si no aplica), total (numero sin formato, igual a amount + tax_iva - withholding_tax. Si no hay desglose, asigna el gran total a este campo total), concept (string resumido del concepto o detalle del gasto), transaction_type (debe ser estrictamente una de estas opciones: "costo_gasto", "inversion" u "otro"). IMPORTANTE: El NIT de la empresa cliente es 902012620-0 (KC Asesorias Urbanas), NO uses este NIT ni este nombre como proveedor, debes extraer los datos del emisor de la factura.'
              }
            ];
