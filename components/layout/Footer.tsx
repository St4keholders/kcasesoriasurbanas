...
export function Footer() {
  const waHref = waUrl(
    `Hola 👋 Quiero información sobre los servicios de ${SITE.name}`
  );

  return (
    <footer className="site-footer">
      <a className="brand" href="#inicio" style={{ fontSize: "0.9rem" }}>
        <svg
          viewBox="0 0 30 30"
        ...
        <span className="brand-name" style={{ fontSize: "0.9rem" }}>
          {SITE.name}
        </span>
      </a>
      <span className="footer-meta">
        Gestión Inmobiliaria · {SITE.city} · © {new Date().getFullYear()}
      </span>
